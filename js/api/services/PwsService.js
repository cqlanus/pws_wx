// @flow
import AmbientWeather from 'ambient-weather-api'
...
export class PwsService implements PwsInterface {
    ...
    api = new AmbientWeather({ apiKey, applicationKey })

    getAllDeviceConditions = () => {}

    getConditions = async () => {
        const devices = await this.api.userDevices()
        const [firstDevice, ...rest] = devices
        const { macAddress } = firstDevice
        const data = await this.api.deviceData(macAddress)
        return data
    }
